...
#page = st.sidebar.radio('菜单栏', ['首页', '图片处理', '聊天', '寻找游戏搭子','听音乐', '智慧词典'])
...
def pagea():
    ...
    st.balloons()
    st.snow()
    st.link_button('找歌网页','http://www.6002255.com/')
def pageb():
    #图片处理
    ...
    if uploaded_file:
        ...
        # tab1,tab2,tab3,tab4,tab5 = st.tabs(['原图','粉红','叶绿','湖蓝','紫罗兰',])
        # with tab1:
        #     st.image(img)
        # with tab2:
...
def pagec():
    ...
    with open('users.txt','r',encoding='utf-8') as t:
        nameword_list = t.read().split('\n')
    for i in range(len(nameword_list)):
        nameword_list[i] = nameword_list[i].split('#')
    dengluname = nameword_list[i][0]
...
def paged():
    ...
    for i in range(len(nameword_list)):
        nameword_list[i] = nameword_list[i].split('#')
        name = nameword_list[i][0]
    my_open = st.toggle('开始匹配')
    if my_open:
        name = nameword_list[i][0]
    else:
        return
    ...
    '选择你的游戏：',
    ['王者荣耀', '原神', '我的世界'],
    captions=['在线人名:' + name, '在线人名:' + name, '在线人名:' + name]
)
